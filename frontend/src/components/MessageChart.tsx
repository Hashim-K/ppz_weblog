...
export function MessageChart({ messages }: MessageChartProps) {
  const [chartType, setChartType] = useState<'timeline' | 'frequency'>('timeline');
  ...
  const frequencyData = () => {
    ...
    messages.forEach(message => {
      typeCounts[message.message_type] = (typeCounts[message.message_type] || 0) + 1;
    });
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Select value={chartType} onValueChange={(value: 'timeline' | 'frequency') => setChartType(value)}>
      ...
      <CardContent>
        ...
        <div className="mt-4 text-sm text-gray-600">
          ...
          {chartType === 'frequency' && (
            <p>
              Showing top {Math.min(frequency.length, 15)} most frequent message types.
            </p>
          )}
        </div>
      </CardContent>
