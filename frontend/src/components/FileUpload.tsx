...
export function FileUpload({ onUploadSuccess, loading, setLoading }: FileUploadProps) {
  const [files, setFiles] = useState<UploadedFiles>({ logFile: null, dataFile: null });
  ...
  const uploadFiles = async () => {
    ...
    try {
      ...
      setError(errorMessage || 'Failed to upload files. Make sure the backend is running on http://localhost:8000');
    } finally {
      setLoading(false);
      setUploadProgress(0);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="text-sm text-gray-600 space-y-1">
        ...
        <ul className="list-disc list-inside space-y-1 ml-4">
          <li><strong>.log file:</strong> Contains aircraft configuration and message definitions</li>
          <li><strong>.data file:</strong> Contains telemetry data from the flight</li>
        </ul>
      </div>
    </div>
  );
