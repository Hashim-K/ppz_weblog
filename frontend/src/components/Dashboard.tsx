...
export function Dashboard({ sessionData }: DashboardProps) {
  ...
  }, [selectedAircraft, selectedMessageType, messageLimit, sessionData, loadMessages]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{sessionData.aircraft_count}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{sessionData.message_count.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {messageStats.timeRange 
                ? `${(messageStats.timeRange.end - messageStats.timeRange.start).toFixed(1)}s`
                : 'N/A'
              }
            </div>
            <p className="text-xs text-muted-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(value) => setSelectedMessageType(value === "all" ? null : value)}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Button onClick={loadMessages} disabled={loading} className="w-full">
      ...
      {selectedAircraftData && (
        <Card>
          <CardHeader>
            <CardTitle>Selected Aircraft: {selectedAircraftData.name}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <p><strong>Aircraft ID:</strong> {selectedAircraftData.id}</p>
              <p><strong>Airframe:</strong> {selectedAircraftData.airframe}</p>
              <div>
                <strong>Available Message Types ({selectedAircraftData.message_types.length}):</strong>
                <div className="flex flex-wrap gap-1 mt-2">
                  {selectedAircraftData.message_types.slice(0, 10).map((type) => (
        ...
        <Card>
          <CardContent className="text-center py-8">
            <p className="text-gray-500">
              No messages found for the selected filters. Try adjusting your selection.
            </p>
          </CardContent>
        </Card>
      )}
