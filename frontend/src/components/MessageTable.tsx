...
export function MessageTable({ messages }: MessageTableProps) {
  ...
  const getFieldSummary = (fields: Record<string, unknown>): string => {
    const fieldNames = Object.keys(fields).filter(key => !key.startsWith('_'));
    return fieldNames.slice(0, 3).join(', ') + (fieldNames.length > 3 ? '...' : '');
  };
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          Showing {messages.length} most recent messages (click to expand fields)
        </p>
      </CardHeader>
      <CardContent>
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="bg-gray-50 p-4">
                          <div className="space-y-2">
                            <h4 className="font-medium text-sm">Message Fields:</h4>
                            ...
                            {message.fields._raw_data ? (
                              <div className="mt-3">
                                <h5 className="font-medium text-xs text-gray-600 mb-1">Raw Data:</h5>
