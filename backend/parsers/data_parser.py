...
class DataParser:
    ...
    def parse_data(self, data_content: bytes, aircraft_config: AircraftConfig) -> List[Message]:
        """
        ...
        while offset < len(data_content):
            try:
                message, bytes_consumed = self._parse_single_message(data_content[offset:])
                if message:
    ...
    def _parse_message_fields(self, data: bytes, fields: List[MessageField]) -> tuple[Dict[str, Any], int]:
