...
class LogParser:
    ...
    def _clean_xml_content(self, content: str) -> str:
        ...
        content = re.sub(r'(<[^>]*=")([^"]*<[^"]*)(");', lambda m: m.group(1) + m.group(2).replace('<', '&lt;') + m.group(3), content)
    ...
    def _parse_aircraft(self, aircraft_elem: ET.Element) -> Optional[Aircraft]:
        ...
        try:
            ...
            radio = aircraft_elem.get("radio", "")
            telemetry = aircraft_elem.get("telemetry", "")
            flight_plan = aircraft_elem.get("flight_plan", "")
            settings = aircraft_elem.get("settings", "")
            gui_color = aircraft_elem.get("gui_color", "#000000")
    ...
    def _parse_message_definition(self, message_elem: ET.Element) -> Optional[MessageDefinition]:
